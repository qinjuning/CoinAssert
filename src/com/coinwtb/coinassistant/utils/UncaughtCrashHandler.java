...
public class UncaughtCrashHandler implements UncaughtExceptionHandler {
	...
	// CrashHandler实例
	private static UncaughtCrashHandler INSTANCE = new UncaughtCrashHandler();
	// 程序的Context对象
	...
	private String saveCrashInfo2File(Throwable ex) {
		...
		try {
			...
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				File crashFolder = Utils.createAppPublicFolderIfRequired("crash");
				if (crashFolder != null) {
					File crashFile = new File(crashFolder, fileName);
				    Log.v(TAG, " crashFile @ " + crashFile.getAbsolutePath());
				    Log.v(TAG, " crashFile @ " + crashFile.getParent());
